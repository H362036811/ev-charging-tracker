...
function installVercel(pkgManager) {
  ...
  try { const result = spawnSync(entry[0], entry[1], { stdio: 'inherit', shell: isWindows }); if (result.status !== 0) throw new Error(`Exit code: ${result.status}`); } catch (error) { log(`Installation failed: ${error.message}`); process.exit(1); }
}
function main() {
  log('========================================');
  ...
  log('');
  checkNode();
  if (checkVercel()) { log('\nVercel CLI is already installed.'); console.log(JSON.stringify({ status: 'already_installed' })); process.exit(0); }
  const pkgManager = detectPackageManager();
  if (!pkgManager) { log('Error: No package manager found'); process.exit(1); }
  installVercel(pkgManager);
  if (checkVercel()) { log('\nVercel CLI installed successfully!'); console.log(JSON.stringify({ status: 'success' })); } else { log('Error: Cannot find vercel after installation'); process.exit(1); }
}
main();
